    ...
    <div style="width: 100%; display: flex; justify-content: center; align-items: center; padding: 20px">
        <div style="display: flex; flex-direction: column; justify-content: center; align-items: center; border: 2px solid #007bff; border-radius: 20px; padding: 20px">
            @if($tenista->imagen != Tenista::$IMAGE_DEFAULT)
                <img alt="Imagen del tenista" class="img-fluid" src="{{ $tenista->imagen_url }}" width="300px" height="300px">
            @else
                <img alt="Imagen por defecto" class="img-fluid" src="{{ Tenista::$IMAGE_DEFAULT }}" width="300px" height="300px">
            @endif
            <form action="{{ route("tenistas.updateImage", $tenista->id) }}" method="post" enctype="multipart/form-data" style="padding-left: 20px; margin-top: 40px">
                @csrf
                @method('PATCH')
